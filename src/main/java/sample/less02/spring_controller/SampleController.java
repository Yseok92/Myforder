...
import com.spring5.mypro00.domain.TodoDTO;
...
public class SampleController {
	...
	//스프링 컨트롤러 클래스 메서드의 @RequestMapping, @GetMapping, @PostMapping
	//어노테이션으로 URLpattern을 지정하는 방법을 실습합니다.
...
//	@RequestMapping(value="/3", method = {RequestMethod.GET, RequestMethod.POST}) // 왜 둘 다 되는지 이해하고 넘어가기, 왜냐하면 mothod에 둘 다 설정했으니까, 그리고 스프링에서는 이렇게 doHandle로 처리하지 않고 
//	public void basic3() {														  // 방식을 미리 잘 파악해서 (Get, Post) Mapping 방식으로 처리함
...
//     ==> 오류
	...
	//	 스프링 컨트롤러 메서드의 매개변수는 사용자 웹브라우저로부터 요청 시에 전달된 값을 저장하는 역할을 합니다.
	...
	public void myEx01(SampleDTO dto) {
		...
		System.out.println("dto.name: " + dto.getAge());
	...
	public void myEx02(String name, int age) { //사용자 요청에서 전달된 값을 받는 역할을 수행
...
//	public void myEx022( String name1,
//						 Integer age1) { 
//		System.out.println("name1: " + name1);
//		System.out.println("age1: " + age1);
//	}//웹브라우저: http://localhost:8080/mypro00/sample/ex022?name=홍길동&age=24 로 요청
	//URL을 보고 name과 age를 참조했으나 같은 이름의 매개변수를 찾지 못해 null로 나오는 결과값
	
	@GetMapping(value="/ex022") // url에 전달된 값의 매개변수 이름과 메서드의 매개변수 이름이 다를 때,
								// @RequestParam 어노테이션으로 url의 매개변수 이름을 설정하여
								// 매개변수에 값을 전달할 수 있습니다.
	...
	@GetMapping(value="/ex02List") // 값 전달이 안됨
	...
	@GetMapping(value="/ex022List") // 값 전달 됨
	...
	@GetMapping(value="/ex02Array") // 값 전달이 됨 ^ㅇ^
	...
	//3. 스프링 컨트롤러 클래스 메서드의 리턴 타입 동작 방식 및 사용법을 실습합니다.
	//   스프링 컨트롤러 메서드의 리턴값은 JSP 페이지를 호출하는 역할을 합니다.
	
	//반환타입: void로 설정된 경우.
	@GetMapping("/ex04")
	public void ex04(SampleDTO dto, Integer page) { // 왜 기본 타입을 못담는다고?
	...
	//브라우저로부터 전달된 값은 컨트롤러 메서드의 매개변수에 자동 저장되고,
	...
	public String myEx05(SampleDTO dto, 
						 @ModelAttribute("page") Integer page1) {
		System.out.println("dto: " + dto);
	...
	public String myEx055(SampleDTO dto, Integer page, Model model) {
		...
		return "sample/ex04" ;
		// 기본 타입, Wrapper, String 타입처럼 전달되지 않는 값들을 JSP에 전달하기 위해 Model 매개변수, addAttribute를 쓰기도 하고
		// @ModelAtrribute 를 사용하기도 합니다. @ModelAttribute는 Model 객체를 만들어서 URL의 값을 전달받아서 뒤의 매개변수에도 전달해 줍니다.
		
		//public String myEx055(SampleDTO dto, Integer page, Model model)
		//model.addAttribute("page", page) ;
		
//		내 정리
		//@ModelAttribute("") 매개변수타입 매개변수
		// 정리하면 @ModelAttribute는 매개변수에 값을 전달할 때 사용하고,
		// 아닌 경우에 Model 매개변수
		//model.addAttribute("", 매개변수) ; 기능을 사용합니다.
		
		
		
	}//웹브라우저: http://localhost:8080/mypro00/sample/ex055?name=홍길동&age=24&page=1 요청
	
	@GetMapping(value="/ex055List") // 값 전달 잘 됨
	...
	//컨트롤러 메서드의 매개변수의 타입은 기본 타입 보다는, Wrapper 클래스의 객체 타입으로 지정하는 것을 권장
	//이유: 전달 받은 값이 없을 경우, String에 의해서 null이 매개변수에 대입될 수 있도록 함
	@GetMapping("/ex06")
	public String myEx06(String name, Integer age, Integer page) {
		...
		//[주의]아래에서 redirect: 다음에 컨텍스트이름을 포함하지 마세요
		
		return "redirect:/ex04.jsp?name=" + name + "&age=" + age + "&page=" + page  ;
		//redirect 가 있으면 bean 설정파일의 @Controller의 영향을 받는 것이 아닌, webapp 이하에서 (컨텍스트 네임을 제외하고) 경로 설정된 파일로 작동합니다
	...
	public String myEx066(String name, Integer age, Integer page,
		...
		//[주의]아래에서 redirect: 다음에 컨텍스트 이름을 포함하지 마세요
	...
	//어떻게 JSP 파일이 호출될까요?, void일 때와 마찬가지로 반환타입이 URL 정보를 기준으로 JSP 파일을 호출
	...
	//반환 타입이 ResponseEntity<E> 인 경우, JSP 호출을 안함
...
//		return new ResposneEntity<String>(myJsonStr, httpHeaders, HttpStatus.OK) ; 
	...
	@GetMapping("/exUpload") //완성된 JSP 페이지 호출 메서드    /WEB-INF/views/sample/exUpload.jsp
	public void exUpload() {
		
	} 
//	http://localhost:8080/mypro00/sample/exUpload
	
	//톰캣 서버상의 업로드 파일이 저장되는 경로 문자열 상수를 선언
	private static final String uploadFolder = "C:/myupload" ;
	
	//*주의사항: MultipartFile 타입의 매개변수 이름은 Form의 file 타입 input의 name 속성과 동일해야 합니다.
	@PostMapping("exUploadPost")
//	public void exUploadpost(MultipartFile myFiles, SampleDTO dto) { //전송 파일이 한 개일 경우
//	public void exUploadpost(MultipartFile[] myFiles, SampleDTO dto) { //전송 파일이 한 개 이상일 경우
	public void exUploadpost(ArrayList<MultipartFile> myFiles, SampleDTO dto) { //전송 파일이 한 개 이상일 경우
...
//		MultipartFile[] 타입 매개변수 사용 시
		...
		for(MultipartFile yourFile : myFiles) {
			...
			if (yourFile.getSize() > 0) {
				System.out.println("서버로 전달된 파일 이름: " + yourFile.getOriginalFilename());
				System.out.println("서버로 전달된 파일 크기: " + yourFile.getSize());
			
			File saveFile = new File(uploadFolder, yourFile.getOriginalFilename()) ;
				try {
					yourFile.transferTo(saveFile);
...
//			System.out.println("서버로 전달된 파일 이름: " + myFile.getOriginalFilename());
//			System.out.println("서버로 전달된 파일 크기: " + myFile.getSize());
//		
